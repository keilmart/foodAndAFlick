...
app.prepRecipeResults = function (recipeResults) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        // get dish types for each recipe, if available
        const dishTypeHtml = app.getDishType(recipe.dishTypes);

        app.printRecipesToPage(recipe, readyTime, dishTypeHtml);
...
app.printRecipesToPage = function (recipe, readyTime, dishTypes) {
    const recipeHtml = `
